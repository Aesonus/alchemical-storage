    ...
    @pytest.mark.parametrize(
        'given_filters, expected_left_list, expected_right_value_list', [
            ...
            (
                ...
                [Model.attr, Model.attr2], ['testvalue', 'testvalue2']
            ),
        ]
    )
    def test_visit_statement_appends_where_clauses_for_given_filters(
            self, mock_sql_statement: Mock,
            expected_left_list, expected_right_value_list, given_filters
    ):
...
class TestOrderByMap:
    ...
    def test_visit_statement_appends_order_by_clauses_for_given_order_by_with_label(
        ...
        """
        Test that the order by map class appends order by clauses for given order by with
        label
        """
        # pylint: disable=comparison-with-callable
        ...
        assert isinstance(
            mock_sql_statement.order_by.call_args.args[1], UnaryExpression)
        assert isinstance(
            mock_sql_statement.order_by.call_args.args[1].element, _textual_label_reference)
        assert mock_sql_statement.order_by.call_args.args[1].element.element == 'exp_label'
        assert mock_sql_statement.order_by.call_args.args[1].modifier == desc_op
    ...
    def test_visit_statement_appends_order_by_clauses_for_given_order_by_with_label_and_attr(
        ...
        """
        Test that the order by map class appends order by clauses for given order by with label
        and attr
        """
        # pylint: disable=comparison-with-callable
